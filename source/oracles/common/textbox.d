...
void initTextbox() {
	if ((textboxFlags & TextboxFlag.dontCheckPosition) == 0) {
		// Decide whether to put the textbox at the top or bottom
		textboxPosition = 2;
		if (link.yh - cameraY >= 72) {
			textboxPosition = 0;
		}
	}
	gbc.SVBK = 7;
...
//	ld d,>w7TextDisplayState
	if (textIsActive == 0xFF) {
		textDisplayMode = 0;
		textbox.displayState = 0x0F;
		textbox.unnamedD0C1 |= 0b1000;
	}
	if (textIsActive != 0x80) {
		//	ld e,<w7TextDisplayState
		switch (textDisplayMode) {
			case 0: standardText(); break;
			case 1: textOption(); break;
			case 2: inventoryText(); break;
			default: assert(0);
		}
	}
	if (textIsActive == 0) {
...
void standardText() {
	switch (textbox.displayState) {
		case 0: // Initializing
			textbox.displayState = 1;
			saveTilesUnderTextbox();
			initTextboxMapping();
			dmaTextboxMap();
			break;
		case 1: // Prepare to draw the top line.
			textbox.displayState++;
			textbox.unnamedD0D3 = 0x40;
			textbox.characterDisplayTimer = textbox.characterDisplayLength;
			drawLineOfText();
			// a is zero after drawLineOfText. Should we consider that a return value...?
			dmaTextGfxBuffer(0);
			break;
		case 2: // displaying top row of characters
		case 4: // displaying bottom row of characters
		case 10: // displaying bottom row of characters, next row will come up automatically
			if (!getNextCharacterToDisplay()) {
				if (!updateCharacterDisplayTimer()) {
					break;
				}
			...
			break;
		case 3: // Preparing to draw the bottom line
		case 9: // Preparing to draw the bottom line (?)
			if (updateCharacterDisplayTimer()) {
				drawLineOfText();
				dmaTextGfxBuffer(2);
				textbox.displayState++;
				textbox.unnamedD0D3 = 0x60;
				textbox.characterDisplayTimer = textbox.characterDisplayLength;
			}
			break;
		case 5: // Waiting for input to display the next 2 rows of characters
			if ((keysJustPressed & (Pad.a | Pad.b)) == 0) {
				updateTextboxArrow();
			} else {
				playSound(Snd.text2);
				textbox.unnamedD0C1 &= ~0b0001;
				goto case 11;
			}
			break;
		case 6: // Nothing?
		case 12: // Nothing?
			...
			dmaTextboxMap();
			break;
		case 7: // Shifts the text up one tile.
		case 13: // Shifts the text up one tile.
			textbox.displayState++;
			shiftTextboxMapUp();
			subFirstRowOfTextMapBy20();
			break;
		case 8: // The first of the next 2 lines of text is about to come up.
			textbox.displayState++;
			textbox.characterDisplayTimer = textbox.characterDisplayLength;
			...
			dmaTextGfxBuffer(0);
			break;
		case 11: // A new line has just been drawn after scrolling text up. Another line of text still needs to scroll up.
			textbox.displayState++;
			// Get the position of the red arrow, remove it
			textbox.map[0x80 + ((textbox.unnamedD0CC + 0x12) & 0x1F)] = 2;

			shiftTextboxMapUp();
			clearTopRowOfTextMap();
			break;
		case 14: // The second new line is ready to be shown.
			textbox.displayState = 3;
			textbox.characterDisplayTimer = textbox.characterDisplayLength;

			dmaTextboxMap();
			dmaTextGfxBuffer(0);
			break;
		case 15: //standardTextStatef
			//	ld h,d
			//	ld l,<w7d0ef
			//	bit 7,(hl)
			//	jr z,@label_3f_096
...
void dmaTextGfxBuffer(ubyte offset) {
	assert(0, "NYI");
//	add $94
//	ld d,a
//	ld e,$00
...
void saveTilesUnderTextbox() {
	copyTileMap(&textbox.map[0], textbox.pos, &textbox.map[0]);
	copyTileMap(&textbox.attributes[0], textbox.pos, &textbox.attributes[0]);
}
...
void copyTileMap(const(ubyte)* src, ubyte index, ubyte* dest) {
	for (int i = 3; i != 0; i--) {
		gbc.SVBK = 7;
		// Copy 2 rows ($40 bytes) to wTmpVramBuffer
		ubyte* tmpDest = &tmpVRAMBuffer[0];
		index &= 0xE0;
		for (int j = 2; j != 0; j--) {
			tmpDest[0 .. 32] = src[index .. index + 32];
			tmpDest += 32;
			index += 64;
		}
		...
		tmpDest = &tmpVRAMBuffer[0];
		for (int j = 2; j != 0; j--) {
			dest[0 .. 32] = tmpDest[0 .. 32];
			tmpDest += 32;
			dest += 64;
		}
	}
...
void initTextboxMapping() {
	ubyte l = (textbox.unnamedD0CC + 1) & 0x1F;
	for (int i = 5; i != 0; i--) {
		ubyte c = l & 0xE0;
		for (int j = 18; j != 0; j--) {
			textbox.map[l] = 2;
			textbox.attributes[l] = 0x80;
			l = ((l + 1) & 0x1F) | c;
		}
		l += 0x20;
	}
...
void dmaTextboxMap() {
	static void common(ubyte size, ushort dest) {
		queueDMATransfer(size, 7, textbox.map[0 .. 10 * 16], toVRAMRange(dest, 0));
		queueDMATransfer(size, 7, textbox.attributes[0 .. 10 * 16], toVRAMRange(dest, 1));
	}
	ubyte c = cast(ubyte)(textMapAddress + 3);
	if (((textbox.vramPos & 0xFF) < 0x61) && (c == 0)) {
		auto dest = textbox.vramPos & 0xFF00;
		ubyte c2 = cast(ubyte)-(textbox.vramPos & 0xFF);
		ubyte b = cast(ubyte)(swap(c2) - 1);
		common(b, textbox.vramPos);
		assert(0, "NYI");
		//common(cast(ubyte)(swap(cast(ubyte)(0xA0 - c)) - 1), );
		//	xor a
		//	sub e
		//	ld c,a
		//	swap a
...
bool getNextCharacterToDisplay() {
	assert(0, "NYI");
//	ld e,<w7NextTextColumnToDisplay
//	ld a,(de)
//	cp $10
