module oracles.common.parentItemUsage;

import oracles.bank00;
import oracles.common.objects.itemParents.commonCode;
...
void bank6FunctionCaller(ubyte func) {
	switch (func) {
		case 0: clearAllParentItemsBody(); break;
		case 1: updateParentItemButtonAssignmentBody(); break;
		case 2: checkUseItems(); break;
		default: assert(0);
	}
}

//; Clears all variables related to items being used?
void clearAllParentItemsBody() {
	foreach (ref itemObject; itemObjects[2 .. 6]) {
		clearParentItem(&itemObject);
	}
	usingShield = 0;
	unnamedCC63 = 0;
	magnetGloveState = 0;
...
//;
void updateParentItemButtonAssignmentBody() {
//	ld h,>w1ParentItem2
//@itemLoop:
//	ld l,Item.enabled
//	ldi a,(hl)
...
//;;
void clearParentItem(GameObject* object) {
	clearLinkUsingItem1(object);
	itemEnableLinkTurning(object);
	itemEnableLinkMovement(object);
	objectDeleteDE(object);
}
