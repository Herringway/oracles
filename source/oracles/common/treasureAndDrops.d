module oracles.common.treasureAndDrops;

import oracles.bank00;
import oracles.common.data.treasureCollectionBehaviours;
import oracles.defs;
...
bool checkTreasureObtainedBody(ubyte id, out ubyte level) {
	if (id >= Treasure.upgrade) {
		if (checkFlag(&upgradesObtained, id & 7)) {
			level = 1;
			return true;
		}
		return false;
	...
	varFF8B = id;
	if (checkFlag(&state.obtainedTreasureFlags[0], id)) {
		auto value = *cast(ubyte*)getValue(treasureCollectionBehaviourTable[varFF8B].relatedVar);
		if (value) {
			level = value;
		}
		return true;
