module oracles.common.objects.interactions.bipinBlossomFamilySpawner;
...
//; ==================================================================================================
void interactionCodeAC(GameObject* obj) {
...
void initializeChildOnGameStart() {
	if (state.childStatus == 0) {
		return;
	}
	state.childStage = 5;
	state.nextChildStage = 5;
	decideInitialChildPersonality();
}
void decideInitialChildPersonality() {
	decideChildPersonalityCommon(&initialChildPersonalityTable[0]);
}

void decideFinalChildPersonality() {
	decideChildPersonalityCommon(&finalChildPersonalityTable[state.childPersonality][0]);
}
void decideChildPersonalityCommon(const(PersonalityRequirement)* table) {
	while (true) {
		if (state.childStatus >= table.minStatus) {
			break;
		}
		table++;
	}
	state.childPersonality = table.personality;
}

struct PersonalityRequirement {
	ubyte minStatus;
	ubyte personality;
}

immutable initialChildPersonalityTable = [
	PersonalityRequirement(11, 0x00), // status >= 11: Hyperactive
	PersonalityRequirement(6, 0x01), // status >= 6:  Shy
	PersonalityRequirement(0, 0x02), // status >= 0:  Curious
];

immutable finalChildPersonalityTable = [
	[ // Hyperactive
		PersonalityRequirement(26, 0x02), // status >= 26: Arborist
		PersonalityRequirement(21, 0x01), // status >= 21: Warrior
		PersonalityRequirement(0, 0x00), // status >= 0:  Slacker
	], [ // Shy
		PersonalityRequirement(19, 0x02), // status >= 19: Arborist
		PersonalityRequirement(15, 0x00), // status >= 15: Slacker
		PersonalityRequirement(0, 0x03), // status >= 0:  Singer
	], [ // Curious
		PersonalityRequirement(14, 0x01), // status >= 14: Warrior
		PersonalityRequirement(10, 0x00), // status >= 10: Slacker
		PersonalityRequirement(0, 0x03), // status >= 0:  Singer
	],
];
