module oracles.common.objects.special.link;
...
//;
//; @param	d	Link object
void specialObjectCodeLink(GameObject* object) {
	switch (object.state) {
		case 0:
			linkState00(object);
			break;
...
//; LINK_STATE_00
void linkState00(GameObject* object) {
	clearAllParentItems();
	specialObjectSetOAMVariables(object);
	specialObjectSetAnimation(object, LinkAnimMode.walk);

	// Enable collisions?
	object.collisionType = 0x80;

	object.collisionRadiusY = 6;
	object.collisionRadiusX = 6;
	// A non-dead default health?
	object.health = 1;
	// Do a series of checks to see whether Link spawned in an invalid position.
	if ((linkForceState != LinkState.warping) && (disableRingTransformations == 0) && (objectGetTileCollisions() == 0xF)) {
		// If he's in a wall, move Link to wLastAnimalMountPointY/X?
		object.yh = state.lastAnimalMountPointY;
		object.xh = state.lastAnimalMountPointX;
	}
	objectSetVisibleC1(object);
	if (!checkLinkForceState(object)) {
		initLinkStateAndAnimateStanding(object);
	}
}
...
//;;
void linkCancelAllItemUsageAndClearAdjacentWallsBitset(GameObject* object) {
	object.adjacentWallsBitset = 0;
	linkCancelAllItemUsage(object);
}
//; Drop any held items, cancels usage of sword, etc?
void linkCancelAllItemUsage(GameObject* object) {
	dropLinkHeldItem();
	clearAllParentItems();
}
...
bool checkLinkForceState(GameObject* object) {
	if (linkForceState != 0) {
		const newState = linkForceState;
		linkForceState = 0;
		linkSetState(object, newState);
		return true;
	}
	return false;
}
...
//; @param	d	Link object
void linkSetState(GameObject* object, ubyte newState) {
	object.state = newState;
	object.substate = 0;
	if ((newState == LinkState.warping) || (newState == LinkState.grabbedByWallmaster) || (newState == LinkState.grabbed)) {
...
void initLinkState(GameObject* object) {
	object.state = LinkState.normal;
	object.substate = 0;
	object.var35 = 0;
}
...
void initLinkStateAndAnimateStanding(GameObject* object) {
	initLinkState(object);
	object.visible |= 0x80;
	animateLinkStanding(object);
}
void animateLinkStanding(GameObject* object) {
	if (object.animMode == LinkAnimMode.walk) {
		if (!checkPegasusSeedCounter()) {
			animateLinkWalking(object);
		}
	}
	// If not using pegasus seeds, set animMode to 0. At the end of the function, this will be changed back to LINK_ANIM_MODE_WALK; this will simply cause the animation to be reset, resulting in him staying on the animation's first frame.
	object.animMode = 0;
	animateLinkWalking(object);
}

void animateLinkWalking(GameObject* object) {
