...
//;
//; @param	c	Amplitude
void initWaveScrollValuesBody(ubyte amplitude) {
	static immutable ubyte[] sineWave = [
...
void loadBigBufferScrollValuesBody(ubyte value) {
//	ld a,:w2WaveScrollValues
//	ld ($ff00+R_SVBK),a
//	ld a,(wFrameCounter)
//	and $7f
...
void resetFollowingLinkPath() {
	...
	followingLinkObject.yh = link.yh;
	followingLinkObject.xh = link.xh;
}

//;;
//; Updates the "FollowingLinkObject" and w2LinkWalkPath if necessary.
//;
void checkUpdateFollowingLinkObject() {
	if (followingLinkObject == null) {
		return;
	}
	// Reset everything if [wLinkPathIndex] >= $80
	if (linkPathIndex >= 0x80) {
		resetFollowingLinkPath();
		return;
	}
	auto hl = &linkWalkPath[linkPathIndex];

	gbc.SVBK = 2;

	// Return if Link's position/direction has not changed
	if ((link.direction == hl.direction) && (link.yh == hl.y) && (link.xh == hl.x)) {
		return;
	}
	// Increment wLinkPathIndex
	linkPathIndex = (linkPathIndex + 1) & 0xF;

	hl = &linkWalkPath[linkPathIndex];

	// Load recorded position values into c/d/e while updating them with Link's new position
	hl.direction = link.direction;
	hl.y = link.yh;
	hl.x = link.xh;
	...
	followingLinkObject.direction = link.direction;
	followingLinkObject.yh = link.yh;
	followingLinkObject.xh = link.xh;
...
//;
void clearMemoryOnScreenReload() {
	clearedOnScreenReload = clearedOnScreenReload.init;
	// Initialize wLinkObjectIndex (set it to >w1Link unless it's already set to >w1Companion).
...
//;;
void func49C9() {
	clearedOnFunc49C9 = clearedOnFunc49C9.init;
	unnamedCCAA = 0xFF;
}
...
void cutscene18() {
	cutscene1819Common(0);
}
//;;
//; CUTSCENE_TWINROVA_SACRIFICE
void cutscene19() {
	cutscene1819Common(1);
}
void cutscene1819Common(ubyte c) {
	assert(0, "NYI");
...
void paletteFadeHandler() {
	switch (paletteThreadMode) {
		case 0: // nothing
			break;
		case 1: // Fade out to white
			varFF8B = 0x1F;
			if (paletteThreadFadeOffset + paletteThreadSpeed >= 32) {
				return paletteThreadStop();
			}
			paletteThreadFadeOffset += paletteThreadSpeed;
			updateFadingPalettes(paletteThreadFadeOffset);
			break;
		case 2: // Fade in from white
			varFF8B = 0x1F;
			if (paletteThreadFadeOffset < paletteThreadSpeed) {
				return paletteThreadStop();
			}
			paletteThreadFadeOffset -= paletteThreadSpeed;
			updateFadingPalettes(paletteThreadFadeOffset);
...
void updateFadingPalettes(ubyte value) {
	paletteThreadCalculateFadingPalettes(value);
	dirtyBGPalettes |= dirtyFadePalettes[0];
	dirtySpritePalettes |= dirtyFadePalettes[1];
	bgPaletteSources = dirtyFadePalettes[2];
	spritePaletteSources = dirtyFadePalettes[3];
}
...
//; @param	c	Value to add to each color component
//; @param	hFF8B	Intensity of a color component after overflowing ($00 or $1f)
void paletteThreadCalculateFadingPalettes(ubyte value) {
	ushort[] palettes = cast(ushort[])tilesetPalettes[];
	ushort[] fadePalettes = cast(ushort[])fadingPalettes[];

	for (int i = 0; i < 0x40; i++) {
		ushort newColour;
		ubyte green = (palettes[i] >> 5) & 0x1F;
		green += value;
		if (green > 0x1F) {
			green = varFF8B;
		}
...
void checkLockBG7Colour3ToBlack() {
	switch(lockBG7Colour3ToBlack) {
		case 0:
			break;
		case 1:
			fadingBGPalettes[0x3E] = 0;
			fadingBGPalettes[0x3F] = 0;
			break;
		default: assert(0);
	}
}
...
bool paletteThreadDecCounter() {
	if (--paletteThreadCounter != 0) {
		return false;
	}
	paletteThreadCounter = paletteThreadCounterRefill;
	return true;
}
...
void func593A() {
	...
	loadCommonGraphics();
	loadGFXRegisterStateIndex(2);
}

void checkUpdateDungeonMinimap() {
	static if (gameVersion == GameVersion.ages) {
		if ((tilesetFlags & (TilesetFlag.largeIndoors | TilesetFlag.sideScroll)) != 0) {
			return;
		}
		if ((tilesetFlags & (TilesetFlag.outdoors | TilesetFlag.dungeon)) == 0) {
			return;
		}
	} else {
		if (activeGroup >= 3) {
			if (((activeGroup & 1) != 0) || (cast(ubyte)(dungeonIndex + 1) == 0)) {
				return;
			}
		}
	}
	state.minimapDungeonFloor = dungeonFloor;
	state.minimapDungeonMapPosition = dungeonMapPosition;
	state.minimapRoom = activeRoom;
	state.minimapGroup = activeGroup;
	// return minimapGroup in c?
}
...
void runGameLogic() {
	switch (gameState) {
		case 0: // Clears a lot of memory, loads common palette header $0f,
			clearOnInitialization = clearOnInitialization.init;
			...
			if ((gameboyType & 0x80) == 0) {
				enum roomSearch1 = gameVersion == GameVersion.ages ? 0x3FE : 0x3AF;
				enum roomSearch2 = gameVersion == GameVersion.ages ? 0x158 : 0x0C5;
				if ((state.deathRespawnBuffer.group == roomSearch1) || ((state.deathRespawnBuffer.group == roomSearch2) && (state.deathRespawnBuffer.x >= 0x40))) {
					loadDeathRespawnBufferPreset(3);
				}
			}
			isLinkedGame = state.fileIsLinkedGame;
			activeGroup = state.deathRespawnBuffer.group;
			activeRoom = state.deathRespawnBuffer.room;
			roomStateModifier = state.deathRespawnBuffer.stateModifier;
			link.enabled = 3;
			linkLocalRespawnDirection = link.direction = state.deathRespawnBuffer.facingDir;
			linkLocalRespawnY = link.yh = state.deathRespawnBuffer.y;
			linkLocalRespawnX = link.xh = state.deathRespawnBuffer.x;
			rememberedCompanionID = state.deathRespawnBuffer.rememberedCompanionID;
			rememberedCompanionGroup = state.deathRespawnBuffer.rememberedCompanionGroup;
			rememberedCompanionRoom = state.deathRespawnBuffer.rememberedCompanionRoom;
			rememberedCompanionY = state.deathRespawnBuffer.rememberedCompanionY;
			rememberedCompanionX = state.deathRespawnBuffer.rememberedCompanionX;
			// Reset health if it's zero or negative
			ubyte initialHealth = state.linkHealth;
			if (cast(byte)initialHealth <= 0) {
				// Set health to wLinkMaxHealth/2 (or 3 hearts minumum)
				initialHealth = (state.linkMaxHealth >> 1) & 0xFC;
				if (initialHealth < 12) {
					initialHealth = 12;
				}
			}
			displayedHearts = state.linkHealth = initialHealth;
			link.invincibilityCounter = 136;
			static if (gameVersion == GameVersion.ages) {
				displayedRupees = state.numRupees;
				loadScreenMusicAndSetRoomPack();
				activeMusic = 0xFF;
				unnamedCC05 = 0xFF;
			} else {
				warpDestPos = getShortPositionFromDE(link.yh);
				loadScreenMusicAndSetRoomPack();
				activeMusic = 0xFF;
			}

			if (checkGlobalFlag(GlobalFlag.pregameIntroDone) != 0) {
				func5A60();
			} else if (checkGlobalFlag(GlobalFlag.unnamed3D) != 0) {
				// The first time the game is opened, this cutscene plays
				gameState = 3;
				link.enabled = 0;
			} else {
				gameState = 2;
				cutsceneIndex = Cutscene.pregameIntro;
				cutscene0D();
			}
			break;
		case 1:
			...
			break;
		case 2:
			if (linkDeathTrigger == 0xFF) {
				playSound(SndCtrl.slowFadeout);
				linkDeathTrigger = 0xE7;
			}
			if (gameOverScreenTrigger != 0) {
				threadRestart(0, &thread1B10);
				return stubThreadStart();
			}
			switch (cutsceneIndex) {
				case 0: cutscene00(); break;
				case 1: cutscene01(); break;
				case 2: cutscene02(); break;
				case 3: cutscene03(); break;
				case 4: cutscene04(); break;
				case 5: cutscene05(); break;
				case 6: cutscene06(); break;
				case 7: cutscene07(); break;
				case 8: cutscene08(); break;
				case 9: cutscene09(); break;
				case 10: cutscene0A(); break;
				case 11: cutscene0B(); break;
				case 12: cutscene0C(); break;
				case 13: cutscene0D(); break;
				case 14: cutscene0E(); break;
				case 15: cutscene0F(); break;
				case 16: cutscene10(); break;
				case 17: cutscene11(); break;
				case 18: cutscene12(); break;
				case 19: cutscene13(); break;
				case 20: cutscene14(); break;
				case 21: cutscene15(); break;
				case 22: cutscene16(); break;
				case 23: cutscene17(); break;
				case 24: cutscene18(); break;
				case 25: cutscene19(); break;

			static if (gameVersion == GameVersion.ages) {
				case 26: cutscene1A(); break;
				case 27: cutscene1B(); break;
				case 28: cutscene1C(); break;
				case 29: cutscene1D(); break;
				case 30: cutscene1E(); break;
				case 31: cutscene1F(); break;
				case 32: cutscene20(); break;
				case 33: cutscene21(); break;
				}
				default: assert(0);
			}
			break;
...
// Sets wEnteredWarpPosition to Link's position, which prevents him from activating a warp tile if he spawns on one.
void setEnteredWarpPosition() {
	enteredWarpPosition = getShortPositionFromDE(link);
}
...
static if (gameVersion == GameVersion.ages) {
	public import oracles.ages.cutscenes;
} else {
	public import oracles.seasons.cutscenes;
}
...
void checkDisplayEraOrSeasonInfo() {
	if (checkGlobalFlag(GlobalFlag.dontDisplaySeasonInfo)) {
		unsetGlobalFlag(GlobalFlag.dontDisplaySeasonInfo);
	}
	if (activeGroup == 0) {
		GameObject* object;
		if (getFreeInteractionSlot()) {
			object.id = Interaction.eraOrSeasonInfo;
		}
	}
}
}
...
void checkPlayRoomMusic() {
	static void setSong(ubyte song) {
		if (activeMusic != song) {
			activeMusic = song;
			playSound(song);
		}
	}
	if (!checkGlobalFlag(GlobalFlag.introDone)) {
		return;
	}
	static if (gameVersion == GameVersion.seasons) {
		// Override subrosia music if on a date with Rosa
		if (checkGlobalFlag(GlobalFlag.datingRosa) && (activeMusic2 == Mus.subrosia)) {
			return setSong(Mus.rosaDate);
		}
	}
	if (activeMusic == 0) {
		return;
	...
	static if (gameVersion == GameVersion.ages) {
		...
		if ((activeMusic2 == Mus.symmetryPresent) && (activeGroup == 0) && ((state.presentRoomFlags[3] & 1) == 0)) {
			activeMusic2 = Mus.sadness;
		}
	}
	setSong(activeMusic2);
}

static if (gameVersion == GameVersion.ages) {
// Seasons has a version of this function a bit higher up.
void checkDisplayEraOrSeasonInfo() {
	if (checkGlobalFlag(GlobalFlag.unnamed16)) {
		unsetGlobalFlag(GlobalFlag.unnamed16);
	}
	if ((sentBackByStrangeForce == 1) || ((tilesetFlags & TilesetFlag.largeIndoors) != 0) || ((tilesetFlags & TilesetFlag.outdoors) == 0)) {
		return;
	}
	GameObject* object;
	if (getFreeInteractionSlot(object)) {
		object.id = Interaction.eraOrSeasonInfo;
	}
}
...
void updateGrassAnimationModifier() {
	static if (gameVersion == GameVersion.ages) {
		grassAnimationModifier = 0;
	} else {
		assert(0, "NYI");
		//	ld a,(wLoadingRoomPack)
		//	inc a
...
void loadDeathRespawnBufferPreset(ubyte index) {
	static if (gameVersion == GameVersion.ages) {
		static immutable respawnBuffers = [
			PartialDeathRespawn(0b11111110, DeathRespawn(0x00, 0xb6, 0x03, 0x02, 0x48, 0x78, 0x00)),
			PartialDeathRespawn(0b11111110, DeathRespawn(0x00, 0x38, 0x00, 0x02, 0x68, 0x50, 0x00)),
			PartialDeathRespawn(0b11011100, DeathRespawn(0x00, 0x6f, 0xff, 0x02, 0x58, 0x78, 0xff)),
			PartialDeathRespawn(0b11011100, DeathRespawn(0x01, 0x58, 0xff, 0x02, 0x48, 0x58, 0xff)),
		];
	} else {
		static immutable respawnBuffers = [
			PartialDeathRespawn(0b11111110, DeathRespawn(0x00, 0xB6, 0x03, 0x02, 0x48, 0x78, 0x00)),
			PartialDeathRespawn(0b11111110, DeathRespawn(0x02, 0x5D, 0x00, 0x02, 0x68, 0x50, 0x00)),
			PartialDeathRespawn(0b11011100, DeathRespawn(0x00, 0x6F, 0xFF, 0x02, 0x58, 0x78, 0xFF)),
			PartialDeathRespawn(0b11011100, DeathRespawn(0x00, 0xC5, 0xFF, 0x02, 0x28, 0x48, 0xFF)),
		];
	}
	enum handleField(string offset, string field) = "if (respawnBuffers[index].fields & (1 << "~ offset ~ ")) { state.deathRespawnBuffer." ~ field ~ " = respawnBuffers[index]." ~ field ~"; }";
	mixin handleField!("0", "group");
	mixin handleField!("1", "room");
	mixin handleField!("2", "stateModifier");
	mixin handleField!("3", "facingDir");
	mixin handleField!("4", "y");
	mixin handleField!("5", "x");
	mixin handleField!("6", "rememberedCompanionID");
	mixin handleField!("7", "rememberedCompanionGroup");
}
...
public import oracles.common.data.seedTreeRefillData;
...
void initializeSeedTreeRefillData() {
	static if (gameVersion == GameVersion.ages) {
		seedTreeRefilledBitset = [0xF0, 0xFF];
	} else {
		seedTreeRefilledBitset = [0xFC];
	}
	gbc.SVBK = 2;
	seedTreeRefillData[] = 0;
	gbc.SVBK = 0;
}
...
static if (gameVersion == GameVersion.ages) {
	...
	public import oracles.ages.underwaterWaves;
	public import oracles.ages.timewarpTileSolidityCheck;

} else {

	public import seasons.onoxCastleEssenceCutscene;
...
public import oracles.common.data.paletteHeaders;
public import oracles.common.data.uncmpGfxHeaders;
public import oracles.common.data.gfxHeaders;
public import oracles.common.data.tilesetHeaders;
...
//	ret
	...
	public import oracles.ages.cutscenes2;
	public import oracles.ages.pirateShip;
