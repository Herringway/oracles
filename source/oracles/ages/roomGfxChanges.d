module oracles.ages.roomGfxChanges;
...
void func027A77() {
	if (checkDungeonUsesToggleBlocks()) {
		return;
	}
	loadUncompressedGfxHeader((toggleBlocksState == 0) ? UGfxH.ages3D : UGfxH.ages3F);
}
...
//;
void applyRoomSpecificTileChangesAfterGfxLoad() {
//	ld a,(wActiveRoom)
...
void checkLoadPastSignAndChestGFX() {
	enum requiredFlags = (TilesetFlag.past | TilesetFlag.largeIndoors | TilesetFlag.dungeon | TilesetFlag.indoors);
	if ((dungeonIndex == 0x0F) || ((tilesetFlags & requiredFlags) != requiredFlags) || ((tilesetFlags & (TilesetFlag.outdoors | TilesetFlag.sideScroll)) != 0)) {
		return;
	}
	loadUncompressedGfxHeader(UGfxH.ages37);
}
