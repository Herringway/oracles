...
void checkDisplayDMGModeScreen() {
	if (gameboyType != 0) {
		return;
	}
	disableLCD();
	loadGFXHeader(GfxH.dmgScreen);
	static if (gameRegion == Region.japan) {
		copyTextCharactersFromSecretTextTable(0, 0x8800);
	}
	...
	while (true) {
		vblankChecker = 0xFF;
		gbc.wait();
		if ((vblankChecker & 0x80) != 0) {
			continue;
		}
		if (serialInterruptBehaviour) {
			serialFunc0C8D();
		}
		serialFunc0C85();
		unnamedFFBE = 3;
...
void updateTilesetFlagsForIndoorRoomInAltWorld() {
//	ld a,(wActiveGroup)
//	or a
//	ret z
...
void copyTextCharactersFromHlUntilNull(ubyte b, ubyte* dest, const(ubyte)* source) {
	copyTextCharactersFromHl(b | 0x80, dest, source);
}
...
void copyTextCharactersFromHl(ubyte length, ubyte* dest, const(ubyte)* source) {
	while (true) {
		ubyte chr = (source++)[0];
		if ((length & 0x80) != 0) {
			if (chr == 0) {
		...
		copyTextCharacterGFX(chr, c, dest);
		if (((length & 0x80) == 0) && (--length == 0)) {
			break;
		}
	}
}

void fileSelectScreen() {
	fileSelect.unnamedCBB6++;
	fileSelectRedrawDecorationsAndSetWramBank4();
	switch (fileSelect.mode) {
		case 0: // Initialization of file select screen
			fileSelect = fileSelect.init;
			disableLCD();
			loadGFXHeader(GfxH.fileMenuGFX);
			playSound(Mus.fileSelect);
			lastSecretInputLength = 0;
			setFileSelectModeTo1();
			goto case;
		case 1:  // Main mode, selecting a file
			{
				switch(fileSelect.mode2) {
					case 0: // Initialization
						setFileSelectCursorOffsetToFileSelectMode();
						func024149(0);
						disableLCD();
						loadGFXHeader(GfxH.fileMenuWithMessageSpeed);
						loadPaletteHeader(PalH.unnamed05);
						loadFileDisplayVariables();
						textInputUpdateEntryCursor();
					...
					case 1: // Normal mode
						...
						ubyte getNextFileSelectMode() {
							if (fileSelect.cursorPos != 3) {
								activeFileSlot = fileSelect.cursorPos;
								if ((getFileDisplayVariableAddress(activeFileSlot, 0).b0 & 0x80) != 0) {
									return 5;
								}
								return 0;
							}
							return cast(ubyte)(fileSelect.cursorPos2 + 3);
						}
						if (fileSelectUpdateInput()) {
							fileSelect.cursorPos2 |= 0x80;
							if ((fileSelect.cursorPos != 3) || func02448D()) {
								break;
							}
						}
						playSound(Snd.selectItem);
						const nextMode = getNextFileSelectMode();
						if (nextMode == 0) {
							// Selected a non-empty file
							incFileSelectMode2();
							loadFile();
							loadUncompressedGfxHeader(UGfxH.unnamed16);
						} else {
							setFileSelectMode(nextMode);
						}
						break;
					case 2: // Selecting text speed
						static void leftOrRight(ubyte offset) {
							if (cast(ubyte)(state.textSpeed + offset) >= 5) {
								return;
							}
							state.textSpeed += offset;
							playSound(Snd.menuMove);
						}
						if ((keysJustPressed & (Pad.b | Pad.select)) != 0) {
							loadUncompressedGfxHeader(UGfxH.unnamed08);
							decFileSelectMode2();
						} else if ((keysJustPressed & Pad.right) != 0) {
							leftOrRight(1);
						} else if ((keysJustPressed & Pad.left) != 0) {
							leftOrRight(0xFF);
						} else if ((keysJustPressed & (Pad.a | Pad.start)) != 0) {
							playSound(Snd.selectItem);
							incFileSelectMode2();
							saveFile();
							fadeoutToWhite();
						}
						textSpeedMenuAddCursorToOAM();
						break;
					case 3: // Screen fading out
						if (paletteThreadMode == 0) {
							// Fade done
							lastSecretInputLength = 0;
							restartThisThread(&mainThreadStart);
						} else {
							textSpeedMenuAddCursorToOAM();
						}
						break;
		...
		case 2:  // Entering name
			switch (fileSelect.mode2) {
				case 0:
					eraseFile();
					loadFile();
					copyNameToW4NameBuffer(0);
					break;
				case 1:
					runTextInput();
					break;
				case 2:
					if (getNameBufferLength() != 0) {
						state.linkName[0 .. 6] = nameBuffer[0 .. 6];
						initializeFile();
					}
					setFileSelectModeTo1();
					break;
		...
		case 4:  // Erase
			...
			assert(0, "NYI");
			break;
		case 5:  // Selecting between new game, secret, link
			switch (fileSelect.mode2) {
				case 0:
					disableLCD();
					loadGFXHeader(GfxH.newFileOptions);
					loadGFXHeader(GfxH.saveMenuLayout);
					loadUncompressedGfxHeader(UGfxH.unnamed08);
					setFileSelectCursorOffsetToFileSelectMode();
					func024149(0);
					loadGFXRegisterState5AndIncFileSelectMode2();
				...
				case 1:
					ubyte c = 1;
					if ((keysJustPressed & Pad.up) != 0) {
						c = 0xFF;
					}
					if (((keysJustPressed & Pad.down) != 0) || ((keysJustPressed & Pad.up) != 0)) {
						do { // repeat +/- 1 until back in range? roughly equivalent to %= 3, I suppose
							fileSelect.cursorPos = (fileSelect.cursorPos + c) & 3;
						} while (fileSelect.cursorPos >= 3);
						playSound(Snd.menuMove);
					} else if ((keysJustPressed & (Pad.b | Pad.select)) != 0) {
						setFileSelectModeTo1();
					} else if ((keysJustPressed & (Pad.a | Pad.start)) != 0) {
						static immutable ubyte[] modes = [
							2, // name entry
							6, // secret entry
							7, // game link
						];
						setFileSelectMode(modes[fileSelect.cursorPos]);
						playSound(Snd.selectItem);
					}
					break;
				default: assert(0);
...
void textSpeedMenuAddCursorToOAM() {
	static immutable cursor = ObjLoad(1, [
		Obj(cast(byte)0x90, 0x31, 0x2e, 0x01),
	]);
	addSpritesToOAMWithOffset(0, cast(ubyte)(state.textSpeed << 4), &cursor);
}

void func024149(ubyte a) {
	fileSelect.cursorPos = a;
	fileSelect.cursorPos2 = 0x80;
	fileSelect.textInputCursorPos = 0x80;
}

void setFileSelectCursorOffsetToFileSelectMode() {
	fileSelect.cursorOffset = fileSelect.mode;
}

void setFileSelectModeTo1() {
	setFileSelectMode(1);
}

void setFileSelectMode(ubyte mode) {
	fileSelect.mode = mode;
	fileSelect.mode2 = 0;
	fileSelect.textInputMode = 0;
}

void loadGFXRegisterState5AndIncFileSelectMode2() {
	loadGFXRegisterStateIndex(5);
	incFileSelectMode2();
}

void incFileSelectMode2() {
	fileSelect.mode2++;
}

void decFileSelectMode2IfBPressed() {
	if ((keysJustPressed & Pad.b) != 0) {
		decFileSelectMode2();
	}
}

void decFileSelectMode2() {
	fileSelect.mode2--;
}
...
FileDisplayVariables* getFileDisplayVariableAddress(ubyte file, ubyte offset) {
	return getFileDisplayVariableAddressParamE(file, offset);
}
FileDisplayVariables* getFileDisplayVariableAddressParamE(ubyte file, ubyte offset) {
	return &fileDisplayVariables[file];
}

//; Returns z-flag unset if something was selected.
bool fileSelectUpdateInput() {
	ubyte a = 0xFF;
	if ((keysJustPressed & Pad.up) == 0) {
		// up not pressed
		a = 1;
		if ((keysJustPressed & Pad.down) == 0) {
			// down not pressed. what about A/start?
			return !(keysJustPressed & (Pad.a | Pad.start));
		}
	}
	fileSelectSetCursor(&fileSelect.cursorPos, (fileSelect.cursorPos + a) & 3);
	fileSelectDrawHeartsAndDeathCounter();
	return true;
}
...
bool func02448D() {
	fileSelect.cursorPos2 &= ~0x80;
	ubyte newPos = 0;
	if ((keysJustPressed & Pad.left) == 0) {
		newPos++;
		if ((keysPressed & Pad.right) == 0) {
			return (keysPressed & (Pad.a | Pad.start)) == 0;
		}
	}
	if (fileSelect.cursorPos2 != newPos) {
		fileSelectSetCursor(&fileSelect.cursorPos2, newPos);
	}
	return true;
}

void fileSelectSetCursor(ubyte* position, ubyte newPosition) {
...
void textInputWaitForInput() {
	if (fileSelect.linkTimer != 0) {
		fileSelect.linkTimer--;
...
void func02461C() {
	loadGFXHeader(GfxH.secretEntryLayout);
	loadUncompressedGfxHeader(UGfxH.unnamed08);
}

//; Returns in b the length of the buffer used. Ignores trailing spaces.
ubyte getNameBufferLength() {
	ubyte result;
	ubyte* name = &nameBuffer[0];
	...
	(name--)[0] = '\0';
	// count backwards until we find a non-space
	for (result = 5; result != 0; result--) {
		ubyte a = cast(ubyte)(name[0] - ' ');
		if (a != '\0') {
			break;
		}
...
unittest {
	nameBuffer[] = 0;
	nameBuffer[0 .. 4] = ' ';
	assert(getNameBufferLength() == 0);
	assert(nameBuffer[0 .. 6] == "\0\0\0\0\0\0");
	nameBuffer[] = 0;
	nameBuffer[0 .. 3] = 'B';
	assert(getNameBufferLength() == 3);
	assert(nameBuffer[0 .. 6] == "BBB\0\0\0");
}

//; Dunno exactly what this does after the jump
//; @param a 1 for kid name, 0 for link name
void copyNameToW4NameBuffer(ubyte name) {
	fileSelect.textInputMode = name;
	if (name == 1) {
		state.kidName[0 .. 6] = nameBuffer[0 .. 6];
	} else {
		state.linkName[0 .. 6] = nameBuffer[0 .. 6];
	}
	fileSelect.textInputMaxCursorPos = 4;
	label02038();
}

void func02465C() {
	if ((secretInputType & 0x80) != 0) {
		lastSecretInputLength = 0xFF;
		fileSelect.textInputMaxCursorPos = 4;
		fileSelect.textInputMode = 0x80;
	} else if (secretInputType != 2) {
		fileSelect.textInputMaxCursorPos = 14;
		fileSelect.textInputMode = 0x81;
	} else {
		fileSelect.textInputMaxCursorPos = 19;
		fileSelect.textInputMode = 0x82;
	}
	label02038();
}
...
void label02038() {
	{
		fileSelect.kanaMode = 0;
		fileSelect.fontXor = 0;
		fileSelect.cursorOffset = 0;
		fileSelect.cursorPos = 0;
		fileSelect.cursorPos2 = 0;
		fileSelect.textInputCursorPos = 0;
		fileSelect.linkTimer = 0;
		fileSelect.unnamedCBC0 = 0;
		fileSelect.unnamedCBC1 = 0;
		fileSelect.unnamedCBC2 = 0;
	}
	textInputLoadCharacterGFX();
	disableLCD();
	loadUncompressedGfxHeader(UGfxH.unnamed0B);
	loadPaletteHeader(PalH.unnamed05);
	if ((fileSelect.textInputMode & 0x80) != 0) {
		if (fileSelect.textInputMaxCursorPos != lastSecretInputLength) {
			secretBuffer[0 .. 32] = ' ';
		}
		loadGFXHeader(GfxH.secretEntryGFX);
		func02461C();
	} else {
		// Entering a name for the player or the kid
		loadGFXHeader(GfxH.nameEntry);
		if ((fileSelect.textInputMode & 1) == 0) {
			// Entering a name for the player

			// Draw the file number
			tileMap[73] = cast(ubyte)(activeFileSlot + ' ');
		}
		loadUncompressedGfxHeader(UGfxH.unnamed08);
	}
	textInputUpdateEntryCursor();
...
void runTextInput() {
	static immutable ubyte[] soundEffects = [
		Snd.selectItem, // a
		Snd.clink, // b
		Snd.selectItem, // select
		Snd.selectItem, // start
		Snd.menuMove, // right
		Snd.menuMove, // left
		Snd.menuMove, // up
		Snd.menuMove, // down
	];
	static void rightLeftCommon(ubyte c) {
		static if (gameRegion == Region.japan) {
			//	ld hl,wFileSelect.cursorPos
			//	ld a,(hl)
			//	cp $50
			...
			ubyte rightmostSpecial = 4;
			ubyte rightmostNormal = 13;
			if ((fileSelect.textInputMode & 0x80) == 0) {
				rightmostSpecial = 3;
				rightmostNormal = 12;
			}
			if (fileSelect.cursorPos < 80) {
				ubyte newXPos = fileSelect.cursorPos;
				do {
					newXPos = (newXPos + c) & 0xF;
				} while (newXPos >= rightmostNormal);
				fileSelect.cursorPos = cast(ubyte)((fileSelect.cursorPos & 0xF0) + newXPos);
				fileSelect.cursorPos2 = 0x80;
			} else {
				ubyte newXPos = fileSelect.cursorPos2;
				do {
					newXPos = (newXPos + c) & 0xF;
				} while (newXPos >= rightmostSpecial);
				fileSelect.cursorPos2 = newXPos;
				textInputLowerOptionUpdateFileSelectCursorPos();
			}
		}
	}
	static void upDownCommon(ubyte c) {
		ubyte newYPos = fileSelect.cursorPos;
		do {
			newYPos = (newYPos + c) & 0x70;
		} while (newYPos >= 0x60);
		fileSelect.cursorPos = cast(ubyte)((fileSelect.cursorPos & 0xF) + newYPos);
		fileSelect.cursorPos2 = 0x80;
		if (fileSelect.cursorPos >= 80) {
			textInputLowerOptionUpdateFileSelectCursorPos2();
		}
	}
	static void selectButtonCommon(ubyte a) {
		//	ld (wFileSelect.kanaMode),a
		//	ld a,(wFileSelect.textInputMode)
		//	rlca
		//	jr c,++
		//	call textInputLoadCharacterGFX
		//	ld a,UNCMP_GFXH_0b
		//	jp loadUncompressedGfxHeader
		//++
		//	xor a
	...
	textInputResult = 1;
	ubyte buttonPressed;
	if (!getHighestSetBit(getInputWithAutofire(), buttonPressed)) {
		return;
	}
	playSound(soundEffects[buttonPressed]);
	switch (buttonPressed) {
		case 0: // a
			if (fileSelect.cursorPos < 80) {
				ubyte b2;
				auto hl = &tileMap[(textInputGetCursorPosition(b2) & 0xF) + 163 + ((b2 >> 4) * 64)];
				static if (gameRegion == Region.japan) {
					assert(0, "NYI");
					//	ld a,(hl)
					//	rrca
					...
					//	dec a
					//	ld e,a
					//	call textInputGetOutputAddress
					//	ld a,(hl)
					//	call func_02_494a
					//	jr c,@gotCharacter
				...
				//@gotCharacter:
				textInputGetOutputAddress()[0] = c;
				goto selectionRight;
			} else {
				if ((fileSelect.textInputMode & 0x80) == 0) {
					switch (fileSelect.cursorPos2) {
						selectionLeft: case 0:
							fileSelect.textInputCursorPos--;
							if (fileSelect.textInputCursorPos >= 0x80) {
								fileSelect.textInputCursorPos = 0;
							}
							textInputUpdateEntryCursor();
							break;
						selectionRight: case 1:
							fileSelect.textInputCursorPos++;
							if (fileSelect.textInputCursorPos >= fileSelect.textInputMaxCursorPos) {
								fileSelect.textInputCursorPos = fileSelect.textInputMaxCursorPos;
							}
							textInputUpdateEntryCursor();
							break;
						case 2:
							static if (gameRegion == Region.japan) {
								selectButtonCommon(1);
							} else {
								goto startButton;
							}
							break;
					...
					switch (fileSelect.cursorPos2) {
						...
						case 2:
							static if (gameRegion == Region.japan) {
								...
								fileSelect.kanaMode = 0;
								fileSelect.cursorPos = 0x57;
								const alreadyOnBack = fileSelect.cursorPos2 == 2;
								fileSelect.cursorPos2 = 2;
								if (alreadyOnBack) {
									fileSelect.mode2 = 3;
								}
							}
			...
			break;
		case 1: // b
			textInputGetOutputAddress()[0] = ' ';
			goto selectionLeft;
		case 2: // select
			static if (gameRegion == Region.japan) {
				selectButtonCommon(fileSelect.kanaMode ^ 1);
			}
			break;
		startButton: case 3: // start
			fileSelect.cursorPos = 0x5A;
			ubyte button = (gameRegion == Region.japan) ? 4 : 2;
			if ((fileSelect.textInputMode & 0x80) != 0) {
				// secret input also has a back button
				button = 3;
			}
			const alreadyOnOK = fileSelect.cursorPos2 == button;
			fileSelect.cursorPos2 = button;
			if (alreadyOnOK) {
				incFileSelectMode2();
			}
			break;
...
// Returns the position of the character within w4TileMap in A, relative to the first character at position $a3.
ubyte textInputGetCursorPosition() {
	ubyte _;
	return textInputGetCursorPosition(_);
}
ubyte textInputGetCursorPosition(out ubyte upper) {
	static if (gameRegion == Region.japan) {
		ubyte offset = 2;
		if ((fileSelect.cursorPos & 0x0F) < 10) {
			offset--;
			if ((fileSelect.cursorPos & 0x0F) < 5) {
				offset--;
			}
		}
	} else {
		...
		if ((fileSelect.cursorPos & 0x0F) < columns) {
			offset = 0;
		}
	}
	upper = fileSelect.cursorPos & 0xF0;
	return cast(ubyte)((fileSelect.cursorPos & 0xF0) + (fileSelect.cursorPos & 0x0F) + offset);
}

// Draws cursors for the currently selected character, and the position in the input string.
void drawNameInputCursors() {
	ubyte position = textInputGetCursorPosition();
	if (position < 80) { // Standard characters
		addSpritesToOAMWithOffset(position & 0xF0, cast(ubyte)((position & 0xF) * 8), &cursorOnCharacterSprites);
	} else { // Extra options like cursor left, cursor right, back, OK
		static if (gameRegion == Region.japan) {
			const(ubyte)[] offsets = jpInputOffsets;
		} else {
			const(ubyte)[] offsets = secretInputOffsets;
			if ((fileSelect.textInputMode & 0x80) == 0) {
				offsets = nameInputOffsets;
			}
		}
		addSpritesToOAMWithOffset(0, offsets[fileSelect.cursorPos2], &lowerOptionCursorSprites);
	}
	addSpritesToOAMWithOffset(0, cast(ubyte)(fileSelect.textInputCursorPos * 8), &textInputCursorSprite);
}
// The cursor and blue highlight for normal characters
immutable cursorOnCharacterSprites = ObjLoad(2, [
	Obj(0x3a, 0x20, 0x2c, 0x02), // Cursor
	Obj(0x38, 0x20, 0x2a, 0x81), // Blue highlight
]);

static if (gameRegion == Region.japan) {
	immutable ubyte[] jpInputOffsets = [ 0x18, 0x30, 0x48, 0x60, 0x78 ];
} else {
	immutable ubyte[] nameInputOffsets = [ 0x18, 0x30, 0x78 ];
	immutable ubyte[] secretInputOffsets = [ 0x18, 0x30, 0x48, 0x60 ];
}

// The cursor for the bottom options
immutable lowerOptionCursorSprites = ObjLoad(2, [
	Obj(cast(ubyte)0x8a, 0x08, 0x2c, 0x01), // Left
	Obj(cast(ubyte)0x8a, 0x10, 0x2c, 0x01), // Right
]);

// The cursor at the top of the screen where the secret/name text is.
immutable textInputCursorSprite = ObjLoad(1, [
	Obj(0x1a, 0x58, 0x2c, 0x82),
]);

//;;
//; Same as drawNameInputCursors, but for secrets.
//drawSecretInputCursors:
//	call textInputGetCursorPosition
//	cp $50
//	jr nc,@lowerOptions
...
void textInputLowerOptionUpdateFileSelectCursorPos() {
	fileSelect.cursorPos = textInputMapUpperXToLowerX(fileSelect.cursorPos2, 0xFF).unnamed00;
}

// Reverse of above
void textInputLowerOptionUpdateFileSelectCursorPos2() {
	fileSelect.cursorPos2 = textInputMapUpperXToLowerX(fileSelect.cursorPos, 0xFF).unnamed01;
}

struct CursorInputMapEntry {
	ubyte unnamed00;
	ubyte unnamed01;
}

CursorInputMapEntry textInputMapUpperXToLowerX(ubyte position, ubyte upper) {
	const(CursorInputMapEntry)* hl = &nameTable[0];
	if ((fileSelect.textInputMode & 0x80) != 0) {
		hl = &secretTable[0];
	}
	while (true) {
		auto entry = (hl++)[0];
		if (entry.unnamed01 == upper) {
			return entry;
		}
		if (entry.unnamed00 == position) {
			return entry;
		}
	}
}
...
static if (gameRegion == Region.japan) {
	immutable nameTable = [
		CursorInputMapEntry(0x50, 0x00),
		CursorInputMapEntry(0x51, 0x00),
		CursorInputMapEntry(0x52, 0x00),
		CursorInputMapEntry(0x53, 0x01),
		CursorInputMapEntry(0x54, 0x01),
		CursorInputMapEntry(0x55, 0x02),
		CursorInputMapEntry(0x56, 0x02),
		CursorInputMapEntry(0x57, 0x02),
		CursorInputMapEntry(0x58, 0x03),
		CursorInputMapEntry(0x59, 0x03),
		CursorInputMapEntry(0x5A, 0x04),
		CursorInputMapEntry(0x5B, 0x04),
		CursorInputMapEntry(0xFF, 0xFF),
	];

	immutable secretTable = [
		CursorInputMapEntry(0x50, 0x00),
		CursorInputMapEntry(0x51, 0x00),
		CursorInputMapEntry(0x52, 0x00),
		CursorInputMapEntry(0x53, 0x01),
		...
		CursorInputMapEntry(0x56, 0x02),
		CursorInputMapEntry(0x57, 0x02),
		CursorInputMapEntry(0x58, 0x02),
		CursorInputMapEntry(0x59, 0x03),
		CursorInputMapEntry(0x5A, 0x03),
		CursorInputMapEntry(0x5B, 0x03),
		CursorInputMapEntry(0xFF, 0xFF),
	];
} else {
	immutable nameTable = [
		...
		CursorInputMapEntry(0x53, 0x01),
		CursorInputMapEntry(0x54, 0x01),
		CursorInputMapEntry(0x55, 0x01),
		CursorInputMapEntry(0x56, 0x02),
		CursorInputMapEntry(0x57, 0x02),
		CursorInputMapEntry(0x58, 0x02),
		CursorInputMapEntry(0x59, 0x02),
		CursorInputMapEntry(0x5A, 0x02),
		CursorInputMapEntry(0x5B, 0x02),
		CursorInputMapEntry(0x5C, 0x02),
		CursorInputMapEntry(0xFF, 0xFF),
	];
...
//; secret input.
void textInputLoadCharacterGFX() {
...
//	ld de,w5NameEntryCharacterGfx
	...
	static if (gameRegion == Region.japan) {
		static immutable ubyte[] unnamed024A28 = [0x2D, 0x20, 0x0E, 0x0F, 0x00];
		ubyte count = 0x2E;
		ubyte offset = 0x60;
		if (fileSelect.kanaMode != 0) {
			offset = 0xB0;
		}
		copyTextCharacters(count, offset, &nameEntryCharacterGFX[0]);
		count = 9;
		const(ubyte)* data = &unnamed024A28[0];
		if ((fileSelect.textInputMode & 0x80) == 0) {
			data++;
			count = 10;
			offset = 0x30;
		...
		copyTextCharacters(count, offset, &nameEntryCharacterGFX[0]);
		copyTextCharactersFromHlUntilNull(count, &nameEntryCharacterGFX[0], data);
	} else {
		if ((fileSelect.textInputMode & 0x80) == 0) {
			copyTextCharacters(0x3B, 0x40, &nameEntryCharacterGFX[0]);
		} else {
			copyTextCharactersFromHlUntilNull(0x40, &nameEntryCharacterGFX[0], cast(immutable(ubyte)*)&secrets[0]);
		}
}
	gbc.SVBK = oldSVBK;
}
...
//; @param	de			Destination
//; @param	wFileSelect.fontXor	Value to xor every other byte with
void copyTextCharacters(ubyte count, ubyte offset, ubyte* destination) {
	while (count-- > 0) {
		copyTextCharacterGFX(offset++, 0, destination);
	}
}
...
void loadFileDisplayVariables() {
	for (activeFileSlot = 2; (activeFileSlot & 0x80) == 0; activeFileSlot--) {
		ubyte c = loadFile();
		auto vars = getFileDisplayVariableAddress(activeFileSlot, 0);
		vars.b0 = c;
		vars.b1 = c;
		vars.numHearts = state.linkMaxHealth;
		vars.numHeartContainers = state.linkMaxHealth;
		vars.deathCount = state.deathCounter;
		vars.b6 = state.fileIsLinkedGame;
		vars.b7 = cast(ubyte)((state.fileIsHeroGame << 1) | state.fileIsCompleted);
		nameBuffer[activeFileSlot * 6 .. (activeFileSlot + 1 ) * 6] = state.linkName[0 .. 6];
	}
	activeFileSlot = 0;
}

//; Updates the displayed text and the cursor?
void textInputUpdateEntryCursor() {
	copyTextCharactersFromHl(24, &gfxBuf[0], textInputGetOutputAddressOffset(0));
	fileSelect.fontXor = 0;
	loadUncompressedGfxHeader(UGfxH.unnamed07);
}

ubyte* textInputGetOutputAddress() {
	return textInputGetOutputAddressOffset(fileSelect.textInputCursorPos);
}
ubyte* textInputGetOutputAddressOffset(ubyte index) {
	if ((fileSelect.textInputMode & 0x80) == 0) {
		return &nameBuffer[index];
	}
	return &secretBuffer[index];
}

void fileSelectDrawHeartsAndDeathCounter() {
	if (fileSelect.mode == 3) {
		return;
	}
	loadGFXHeader(GfxH.fileMenuLayout);

	// Jump if cursor isn't on a file
	if ((fileSelect.cursorPos < 3) && ((getFileDisplayVariableAddress(fileSelect.cursorPos, 0).b0 & 0x80) == 0)) {
		// Draw death count
		auto deathCount = getFileDisplayVariableAddressParamE(fileSelect.cursorPos, 4).deathCount;
		tileMap[0x130] = cast(ubyte)((deathCount & 0xF) + 16);
		tileMap[0x12F] = cast(ubyte)(((deathCount & 0xF0) >> 4) + 16);
		tileMap[0x12E] = cast(ubyte)(((deathCount & 0xF00) >> 4) + 16);

		// Draw hearts
		fileSelectDrawHeartDisplay(getFileDisplayVariableAddress(fileSelect.cursorPos, 2).numHeartContainers, getFileDisplayVariableAddress(fileSelect.cursorPos, 2).numHearts, &tileMap[0x14A]);
	}
...
void fileSelectDrawAcornCursor() {
	static immutable sprite = ObjLoad(1, [
		Obj(0x10, 0x08, 0x28, 0x04),
	]);
	static struct CursorPos {
		ubyte x;
		ubyte y;
	}
	immutable CursorPos[][2][] cursorPositions = [
		[
			[
				CursorPos(0x34, 0x50),
				CursorPos(0x4C, 0x50),
	...
	static void func(const(CursorPos)* pos, const(ObjLoad)* sprite) {
		addSpritesToOAMWithOffset(pos.x, pos.y, sprite);
	}
	if ((fileSelect.cursorPos & 0x80) == 0) {
		func(&cursorPositions[fileSelect.cursorOffset][0][fileSelect.cursorPos], &sprite);
	}
	if ((fileSelect.cursorPos2 & 0x80) == 0) {
		func(&cursorPositions[fileSelect.cursorOffset][1][fileSelect.cursorPos2], &sprite);
	}
}
...
void fileSelectRedrawDecorationsAndSetWramBank4() {
	static immutable sprites = ObjLoad(16, [
		Obj(cast(byte)0x23, cast(byte)0x0A, 0x20, 0x05),
		Obj(cast(byte)0x23, cast(byte)0x12, 0x22, 0x05),
		Obj(cast(byte)0x33, cast(byte)0x06, 0x20, 0x05),
		Obj(cast(byte)0x33, cast(byte)0x0E, 0x22, 0x05),
		Obj(cast(byte)0x0F, cast(byte)0x07, 0x26, 0x05),
		Obj(cast(byte)0x3B, cast(byte)0x16, 0x20, 0x25),
		Obj(cast(byte)0x3B, cast(byte)0x0E, 0x22, 0x25),
		Obj(cast(byte)0x17, cast(byte)0x0A, 0x24, 0x25),
		Obj(cast(byte)0x21, cast(byte)0x96, 0x20, 0x05),
		Obj(cast(byte)0x21, cast(byte)0x9E, 0x22, 0x05),
		Obj(cast(byte)0x17, cast(byte)0x9B, 0x26, 0x65),
		Obj(cast(byte)0x14, cast(byte)0x9D, 0x24, 0x05),
		Obj(cast(byte)0x31, cast(byte)0xA2, 0x20, 0x25),
		Obj(cast(byte)0x31, cast(byte)0x9A, 0x22, 0x25),
		Obj(cast(byte)0x39, cast(byte)0x92, 0x20, 0x05),
		Obj(cast(byte)0x39, cast(byte)0x9A, 0x22, 0x05),
	]);
	clearOAM();
	gbc.SVBK = 4;
	addSpritesToOAM(&sprites);
}


void fileSelectDrawLinkInOtherGame() {
	static if (gameVersion == GameVersion.ages) {
		enum sprite = 0;
	} else {
		enum sprite = 4;
	}
	fileSelectDrawLinkCommon(sprite);
}

//; Draws link and nayru/din on the file select
void fileSelectDrawLink() {
	static if (gameVersion == GameVersion.ages) {
		enum sprite = 4;
	} else {
		enum sprite = 0;
...
void fileSelectDrawLinkCommon(ubyte sprite) {
	static immutable spriteTable = [
	// Seasons (frame 0)
		fileSelectSprite0, // $00 - link standing still (blank file)
		fileSelectSprite3, // $01 - linked file (rod of seasons)
		fileSelectSprite7, // $02 - completed file (with din)
		fileSelectSprite1, // $03 - unlinked file
	// Ages (frame 0)
		fileSelectSprite0, //$04 - link standing still (blank file)
		fileSelectSprite5, //$05 - linked file (harp of ages)
		fileSelectSprite9, //$06 - completed file (with nayru)
		fileSelectSprite1, //$07 - unlinked file
	// Seasons (frame 1)
		fileSelectSprite0,
		fileSelectSprite4,
		fileSelectSprite8,
	...
	// Ages (frame 1)
		...
		fileSelectSpriteA,
		fileSelectSprite2,
		fileSelectSpriteB, // $10 - Triforce symbol for hero's file
	];
	void draw(ubyte a, ubyte b) {
		addSpritesToOAM(&spriteTable[a + b]);
	}
	if (fileSelect.cursorPos >= 3) {
		return;
	}
	ubyte c = 0;
	if ((getFileDisplayVariableAddress(fileSelect.cursorPos, 0).b0 & 0x80) == 0) {
		// Draw triforce symbol for hero's files
		if ((getFileDisplayVariableAddressParamE(fileSelect.cursorPos, 7).b7 & 0b0010) != 0) {
			draw(0, 16);
		}
		c++;
		// Check whether file is linked
		if ((getFileDisplayVariableAddressParamE(fileSelect.cursorPos, 6).b6 & 1) == 0) {
			c++;
			// Not linked; check whether the file is completed
			if ((getFileDisplayVariableAddressParamE(fileSelect.cursorPos, 7).b7 & 1) == 0) {
				c++;
			}
		}
	}
	if ((fileSelect.unnamedCBB6 & 0x10) == 0) {
		draw(c, sprite);
	} else {
		draw(cast(ubyte)(c + 8), sprite);
	}
}


immutable fileSelectSprite0 = ObjLoad(2, [
	Obj(0x4e, 0x58, 0x04, 0x00),
	Obj(0x4e, 0x60, 0x06, 0x00),
]);

immutable fileSelectSprite1 = ObjLoad(2, [
	Obj(0x4e, 0x58, 0x00, 0x00),
	Obj(0x4e, 0x60, 0x02, 0x00),
]);

immutable fileSelectSprite2 = ObjLoad(2, [
	Obj(0x4e, 0x58, 0x02, 0x20),
	Obj(0x4e, 0x60, 0x00, 0x20),
]);

immutable fileSelectSprite3 = ObjLoad(4, [
	Obj(0x4e, 0x58, 0x0a, 0x20),
	Obj(0x4e, 0x60, 0x08, 0x20),
	Obj(0x4e, 0x63, 0x1c, 0x22),
	Obj(0x4e, 0x6b, 0x1a, 0x22),
]);

immutable fileSelectSprite4 = ObjLoad(4, [
	Obj(0x4e, 0x58, 0x0e, 0x20),
	Obj(0x4e, 0x60, 0x0c, 0x20),
	Obj(0x4e, 0x68, 0x1c, 0x22),
	Obj(0x4e, 0x70, 0x1a, 0x22),
]);

immutable fileSelectSprite5 = ObjLoad(3, [
	Obj(0x4e, 0x58, 0x12, 0x20),
	Obj(0x4e, 0x60, 0x10, 0x20),
	Obj(0x4e, 0x64, 0x14, 0x22),
]);

immutable fileSelectSprite6 = ObjLoad(3, [
	Obj(0x4e, 0x58, 0x18, 0x20),
	Obj(0x4e, 0x60, 0x16, 0x20),
	Obj(0x4e, 0x64, 0x14, 0x22),
]);

immutable fileSelectSprite7 = ObjLoad(5, [
	Obj(0x4e, 0x58, 0x00, 0x00),
	Obj(0x4e, 0x60, 0x02, 0x00),
	Obj(0x4e, 0x68, 0x00, 0x0a),
	Obj(0x4e, 0x70, 0x02, 0x0a),
	Obj(0x3e, 0x6d, 0x04, 0x0a),
]);

immutable fileSelectSprite8 = ObjLoad(5, [
	Obj(0x4e, 0x58, 0x02, 0x20),
	Obj(0x4e, 0x60, 0x00, 0x20),
	Obj(0x4e, 0x68, 0x02, 0x2a),
	Obj(0x4e, 0x70, 0x00, 0x2a),
	Obj(0x3e, 0x6b, 0x04, 0x2a),
]);

immutable fileSelectSprite9 = ObjLoad(4, [
	Obj(0x4e, 0x58, 0x00, 0x00),
	Obj(0x4e, 0x60, 0x02, 0x00),
	Obj(0x4e, 0x68, 0x06, 0x09),
	Obj(0x4e, 0x70, 0x08, 0x09),
]);

immutable fileSelectSpriteA = ObjLoad(4, [
	Obj(0x4e, 0x58, 0x02, 0x20),
	Obj(0x4e, 0x60, 0x00, 0x20),
	Obj(0x4e, 0x68, 0x08, 0x29),
	Obj(0x4e, 0x70, 0x06, 0x29),
]);

immutable fileSelectSpriteB = ObjLoad(2, [
	Obj(0x4a, cast(byte)0x8c, 0x30, 0x06),
	Obj(0x4a, cast(byte)0x94, 0x32, 0x06),
]);
...
void runBank2Function(ubyte func) {
//	ld c,l
//	ld a,h
//	rst_jumpTable
...
void fileSelectDrawHeartDisplay(ubyte heartContainers, ubyte hearts, ubyte* tileBuffer) {
	varFF8B = 1;
	drawHeartDisplay(heartContainers, hearts, tileBuffer);
}
...
//; @param hFF8B
void drawHeartDisplay(ubyte heartContainers, ubyte hearts, ubyte* tileBuffer) {
	...
	//; @param hl Tile buffer to write to
	static void drawHeartDisplayRow(ubyte unfilledHearts, ubyte filledHearts, ubyte unfilledQuarters, ubyte* tileBuffer) {
		if (filledHearts != 0) {
			while (filledHearts-- != 0) {
				(tileBuffer++)[0] = 15;
			}
		}
		if (unfilledHearts != 0) {
			if (unfilledQuarters != 0) {
				(tileBuffer++)[0] = cast(ubyte)(unfilledQuarters + 11);
				unfilledHearts--;
			}
			if (unfilledHearts != 0) {
				while (unfilledHearts-- != 0) {
					(tileBuffer++)[0] = 11;
				}
			}
		}
		if (varFF8B == 0) {
			for (ubyte c = 8; c != 0; c--) {
				(tileBuffer++)[0] = varFF8B;
			}
		}
	}
	// hearts per row (7 normally, 8 if you have 15+ hearts)
	ubyte heartsPerRow = (heartContainers > 14 * 4 + 1) ? 7 : 8;
	ubyte b = heartContainers / 4;
	ubyte d = hearts & 3;
	ubyte c = hearts / 4;
	ubyte tmpC = c;
	if (tmpC >= heartsPerRow) {
		tmpC = heartsPerRow;
	}
	ubyte tmpA = b;
	if (tmpA >= heartsPerRow) {
		tmpA = heartsPerRow;
	}
	ubyte tmpB = cast(ubyte)(tmpA - tmpC);
	if (((varFF8B | heartsPerRow) & 1) == 0) {
		tileBuffer--;
	}
	drawHeartDisplayRow(tmpB, tmpC, d, tileBuffer);
	...
	drawHeartDisplayRow(b, c, d, tileBuffer);
}
...
static if (gameVersion == GameVersion.ages) {
	public import oracles.common.roomInitialization;
	public import oracles.common.roomGfxChanges;
}
